...
`event_time` 을 DATE 로 변경한 후 Group By 해서 일별 구매 이벤트 숫자를 집계해보면 다음과 같습니다. 이 과정에서 구매 이벤트만 필터링 하기 위해 [`where`](2.1.5-spark-dataframe.md#basic-and-aggregation) 함수를 사용합니다.
...
![Attribution Model Overview (https://www.silverdisc.co.uk/blog/2017/09/22/attribution-modelling-%E2%80%93-moving-away-last-click-attribution)](<../../.gitbook/assets/image (5) (1).png>)
...
* [Spark Docs - Spark Window Function](2.1.5-spark-dataframe.md#basic-and-aggregation)
...
![Session Overview (Google Analytisc)](<../../.gitbook/assets/image (6) (1).png>)
...
\[그림] Kafka Event Server 의 경우 Avro, ProtoBuf Binary 그러나 Client 의 경우 JSON\
...
category\_parsed 컬럼이 struct 타입임을 볼 수 있습니다.



{% hint style="info" %}
AWS 에서는 ALB, CLB, CloudTrail 등의 로그가 S3 에 JSON 형식으로 쌓입니다. (JSON.gz)

이 데이터를 spark.read.json() 으로 읽어서 원하는 컬럼을 뽑아내는 연습을 해봅시다.&#x20;

* CloudTrail 과 같은 경우에는 특정 컬럼의 형식이 Event Type 에 따라 동적일 수 있습니다.&#x20;
* 그렇다면 StructType 이 나을지, get\_json\_object 가 나을지도 논의해 봅시다.
{% endhint %}

\[TASK] S3 샘플 제공하기 (ALB, CloudTrail)
